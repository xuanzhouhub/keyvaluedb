...
bool LSMTreeEngine::CompareAndSwap(const std::string& key,
    ...
    uint64_t write_ts;
    {
        ...
        while (batch_in_progress_ && global_ts_.load() + 1 > batch_ts_)
            batch_cv_.wait(lock);
        write_ts = global_ts_.fetch_add(1) + 1;
    }

    std::string current;
    bool found = false;
    std::thread read_thread([this, &key, &current, &found]() {
        found = Lookup(key, current);
    });
    read_thread.join();
    ...
    wal_->Buffer(key, desired, write_ts);
    size_t my_seq = wal_->CurrentBatchSeq();
    ...
    {
        std::unique_lock<std::shared_mutex> lock(memtable_mutex_);
        active_memtable_->Insert(key, desired, write_ts, false);
        if (active_memtable_->IsFull()) {
            auto frozen = active_memtable_;
            frozen->Freeze();
            frozen_memtables_.push_back(frozen);
    ...
    kv_cache_->Put(key, desired, write_ts);
